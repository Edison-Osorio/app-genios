...
export class ListadoGeniosComponent implements OnInit {
  ...
  constructor(private readonly listadoGeniosService: ListadoGeniosService) {}

  ngOnInit(): void {
    this.getlistadoGenios();
  }

  private getlistadoGenios(): void {
    this.listadoGeniosService.obtenerGenios().subscribe({
      next: (genios: Genio[]) => (this.listadoGenios = genios),
      error: (error: ErrorHandler) =>
        Swal.fire('Error', ` ¡Ho no! ha ocurrido un error ${error}`, 'error'),
    });
  }
}
